...
function loadMenu(data, mealType, searchWord) {
  ...
  if (searchWord.length === 0) {
    ...
    for (const [category, menuItems] of Object.entries(data[`${mealType}Menu`])) {
      ...
      for (let pizzaName in menuItems.items) {
        // let pizza = menuItems.items[pizzaName]; // Access the value (details) of each item

        let isInMenuItem = pizzaName.toLowerCase().includes(searchWord.toLowerCase());
        // if (isInMenuItem) {
    ...
    //   return;
    // }
    //  we can pass category as arugment in addition to searchMenuItems
  ...
  function renderMenuItems(jsonMenu, element, searchWord = '') {
    ...
    for (let category in jsonMenu) {
      ...
      if (searchWord.length > 0) {
        ...
        // Loop through the new keys and update the object
        for (let i = 0; i < highlightedItemsResult.length; i++) {
          const oldKey = Object.keys(items)[i]; // Get old key based on index
          const newKey = highlightedItemsResult[i];

          // Check if the old key exists before updating
          if (items.hasOwnProperty(oldKey)) {
            items[newKey] = items[oldKey]; // Copy properties to new key
            delete items[oldKey]; // Remove the old key
          } else {
            console.warn(`Key "${oldKey}" not found in the menu. Skipping update.`);
          }
        }
